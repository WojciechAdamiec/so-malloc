...
static void *extend_heap(size_t words) {
  ...
  PUT(HDRP(bp), PACK_WITH_PREV(size, FREE, last_prev_alloc));
  PUT(FTRP(bp), PACK_WITH_PREV(size, FREE, last_prev_alloc));
...
static void place(void *bp, size_t asize) {
  ...
  if ((csize - asize) >= (ALIGNMENT)) {
    PUT(HDRP(bp), PACK_WITH_PREV(asize, ALLOCATED, ALLOCATED));
    bp = NEXT_BLKP(bp);
    PUT(HDRP(bp), PACK_WITH_PREV(csize - asize, FREE, ALLOCATED));
    PUT(FTRP(bp), PACK_WITH_PREV(csize - asize, FREE, ALLOCATED));
    // printf("Place with split, next bp=%p\n", bp);
  } else {
    PUT(HDRP(bp), PACK_WITH_PREV(csize, ALLOCATED, ALLOCATED));
